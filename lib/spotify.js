...
const getAccessToken = async () => {
  ...
  // Spotify answers a revoked/rotated credential with a 4xx and an error body.
  // Returning that as-is leaves access_token undefined, so every downstream
  // call goes out as "Bearer undefined" and comes back 401 -- which hides the
  // real cause. Surface it here instead.
  if (!response.ok || !data.access_token) {
    throw new Error(
      `Spotify token refresh failed: ${response.status} ${data.error || 'unknown_error'}` +
